...
class GDA:
    ...
    def get_client(image):
        ...
        for xy in GDA.TEST+GDA.labb:
            gray_array[xy[1]][xy[0]]=(255,0,0,255)
    ...
    def __get_loops_from_image(image):
        ...
        while x != len(gray_array[0])-1:
            if GDA.__delta_pixel((x, y), gray_array) > GDA.SEPARATION_CONSTANT:
                position = (x, y)
                is_loop = GDA.__find_delta_loop(gray_array, position)
                if is_loop:
                    GDA.TEST += [position]
                    loop_image, biggest_x_value_for_cord_y = GDA.__extract_loop((x, y), gray_array)
                    loop_images += [loop_image]
                    x = biggest_x_value_for_cord_y
            x += 1
        for image1 in loop_images:
            image1.show()
