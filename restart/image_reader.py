...
def get_mana():
    i = open('stats.txt','r')
    return np.array([int(num) for num in i])
...
def create_model():
    ...
    # x_train = [np.asarray(Image.open(i)) for i in get_img_names()]
    x_train = get_img()
    y_train = get_mana()
    ...
    y_train = tensorflow.keras.utils.to_categorical(y_train, NUM_CLASSES+1)
